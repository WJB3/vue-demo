...
export default  {
    ...
    actions:{
        async add({commit,dispatch},payload){
            ...
            try{    
                const list=await activityService.add(payload);
                dispatch("getList");
                if(list){
        ...
        async edit({commit,dispatch},payload){
            try{    
                const list=await activityService.edit(payload);
                dispatch("getList");
                if(list){
                    return true;
                }
        ...
        async delete({commit,dispatch},payload){//品牌删除
            try{    
                const list=await activityService.deleteBrand(payload);
                dispatch("getList")
            }catch(e){
                console.error(e);
            }
        ...
        async getList({commit},payload){//用户登录
            ...
            try{    
                const list=await activityService.list(payload);
                commit("updateState",{
                    loading:false
                })
                commit("updateState",{
